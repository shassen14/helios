...
//! the pipeline's latest control command (a `BodyTwist`) into
//! `ControlOutputComponent`, which the vehicle adapter consumes in `Actuation`.
//! The controller itself is a DAG node inside `BrainTick`; this is only the
//! bus→ECS bridge.
...
/// Copies the pipeline's latest control command (written by the controller
