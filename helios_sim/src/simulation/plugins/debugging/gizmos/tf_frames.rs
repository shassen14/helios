...
pub fn draw_tf_frames(
    ...
    for (_entity, world_iso, _local_iso, parent_entity) in tf_tree.iter_frames() {
        ...
        let axis_len = if parent_entity.is_none() {
            0.8_f32
        } else {
            0.5_f32
        };
        ...
        gizmos.arrow(
            frame_origin,
            frame_origin + rot * Vec3::X * axis_len,
            Color::srgb(1.0, 0.2, 0.2),
        );
        gizmos.arrow(
            frame_origin,
            frame_origin + rot * Vec3::Y * axis_len,
            Color::srgb(0.2, 1.0, 0.2),
        );
        gizmos.arrow(
            frame_origin,
            frame_origin + rot * Vec3::Z * axis_len,
...
pub fn update_tf_labels(
    ...
    for (frame_entity, _, _, _) in tf_tree.iter_frames() {
        let Some(frame_name) = tf_tree.entity_to_name.get(&frame_entity) else {
            continue;
        };
    ...
    for (frame_entity, world_iso, _, _) in tf_tree.iter_frames() {
        ...
        let Some(&label_entity) = labels.0.get(frame_name.as_ref()) else {
            continue;
        };
        let Ok((mut node, mut vis)) = node_query.get_mut(label_entity) else {
            continue;
        };
