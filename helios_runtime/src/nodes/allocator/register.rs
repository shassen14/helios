...
use helios_core::control::{
    ...
    allocation::wheeled::{
        drive_torque::WheelTorqueAllocator, steer_position::SteerPositionAllocator,
    },
};
...
fn build_wheel_torque(ctx: AllocatorBuildContext) -> Result<Box<dyn PipelineNode>, String> {
    ...
    // `kind` string, so `ctx.config` is already this variant. It consumes a
    // `DriveForce` and emits a single wheel-torque setpoint (`τ = F·r`); it pairs
    // with a lateral allocator on a steered vehicle. The `else` arm can only fire
    // if the registry maps this kind to the wrong factory — a wiring mistake, not
    // bad user config — so it errors loudly and names what arrived.
