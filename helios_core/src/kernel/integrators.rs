// helios_core/src/utils/integrators.rs
...
where
    T: Copy + Sub<Output = T> + Mul<DVector<T>, Output = DVector<T>> + Float,
    DVector<T>: Add<Output = DVector<T>>,
{
    fn step(
        &self,
        func: &dyn Fn(&DVector<T>, T) -> DVector<T>,
...
where
    T: Copy + Float + Mul<DVector<T>, Output = DVector<T>>,
    DVector<T>: Add<Output = DVector<T>>,
{
    fn step(
        &self,
