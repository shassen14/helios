...
- **Complex Colliders:** Terrain tiles and world objects with complex geometry use `Collider::trimesh_from_mesh_with_config(mesh, TrimeshFlags::FIX_INTERNAL_EDGES)`. The visual mesh and the collision mesh are separate GLB files — this lets artists author a simplified low-poly collision hull for performance without touching the visual asset. The collision GLB path is specified in the scenario TOML under `terrain.collider` or in a world-object prefab under `collider_mesh`. Both are loaded during `AppState::AssetLoading` and consumed during `AppState::SceneBuilding`. Primitive colliders (`box`, `sphere`, `capsule`, `cylinder`) are also available for simple objects via the `[collider]` table in a world-object prefab; `collider_mesh` takes priority if both are present.
